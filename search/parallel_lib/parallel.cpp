...
{
    int pp[2], n = vec.size(), procs = get_nprocs(), start, finish, argmin_i, argmin, ans;
    ...
    for(int i = 0; i < procs; i++) 
    {
        if(fork() == 0)
        {
            ...
            write(pp[1], &argmin_i, sizeof(argmin_i));
    ...
    { 
        close(pp[1]);
        read(pp[0], &argmin, sizeof(argmin));
        wait(&status);
        //printf("Parent received value: %d, ", argmin[i]);
        //printf("%d \n", WEXITSTATUS(status));
        dist = my_euclid_dist(vec[argmin]);
        if (minimal > dist)
        {
            minimal = dist;
            ans = argmin;
